...
trait CustomMetaboxes {
	...
	static function parentInit() {
		\add_action('add_meta_boxes_' . self::PREFIX, [__CLASS__, 'addMetaBoxes'], 10, 1);
		\add_action('edit_form_after_title', [__CLASS__, 'addPostTitleBoxes'], 10, 1);
		\add_action('save_post', [__CLASS__, 'saveMetaKeys'], 10, 1);
		//\add_action('wp_ajax_autosave_' . self::PREFIX . '_meta', [__CLASS__, 'handleAutosave']);
	 ...
	 * @param  boolean $save_tags If HTML should be preserved
	 * @param  integer $post_id  Post ID
	 * @return mixed             Sanitized value
	 */
	static function sanitizeValue($type = 'string', $value, $save_tags = false, $post_id = 0) {
		switch($type) {
			case 'url':
				return (string) esc_url_raw($value);
				break;
			case 'textarea':
				return $save_tags ? (string) self::stripWhitespace($value, $post_id) : (string) \sanitize_textarea_field($value);
				break;
			case 'int':
			case 'integer':
			case 'featured_image':
				...
				break;
			default:
				return $save_tags ? (string) self::stripWhitespace($value, $post_id) : (string) \sanitize_text_field($value);
		}
	}
	 ...
	 * @return void
	 */
	static function saveMeta($post_id, $key, $type = 'string', $save_tags = false) {
		if ( ! self::safeToSave()) {
			return;
		}
			...
			if ($type === 'multi') {
				...
				foreach ($value as $subkey=>$subvalue) {
					...
					if (array_key_exists($subkey, self::$metakeys[$key]->subtypes)) {
						...
						$value[$subkey] = self::sanitizeValue($subtype->type, $value[$subkey], $subtype->save_tags, $post_id);
					} else {
						unset($value[$subkey]);
					}
				}
				...
				$value = self::sanitizeValue($type, $value, $save_tags, $post_id);
			}
	...
	static function saveMetaKeys($post_id) {
		...
		foreach (self::$metakeys as $metakey) {
			self::saveMeta($post_id, $metakey->key, $metakey->type, $metakey->save_tags);
		}
	}
	...
	static function handleAutosave() {
		...
		if ( ! isOurPost($_POST['post_ID'])) {
			\wp_send_json(['success' => false, 'msg' => self::PREFIX . ' autosave called outsite of ' . self::PREFIX]);
		}
	...
	static function outputType($meta, $value, array $options = array()) {
		...
		switch($meta->type) {
			...
			case 'checkbox':
				...
				break;
			case 'radio':
				return self::displayField_RADIO($meta, $value, $options);
				break;
			case 'select':
				return self::displayField_SELECT($meta, $value, $options);
				break;
			case 'featured_image':
			...
			case 'multi':
				...
				foreach ($value as $subkey=>$subvalue) {
					if (array_key_exists($subkey, $meta->subtypes)) {
						...
						if ($submeta->type === 'checkbox' && ! $submeta->multiselect) {
							$submeta->key = $meta->key;
						} else {
